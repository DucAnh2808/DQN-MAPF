...
class Agent:
    def __init__(self, policy_net, target_net, device):
        ...
        """learning rate"""
        self.lr_start = 0.001 #Doi lai thanh 0.01
        ...
        self.optim = torch.optim.Adagrad(self.policy_network.parameters(), self.lr)  # 效果好，lr=0.01  # self.lr learning rate la toc do hoc cua mo hinh
        ...
        self.possible_actions = np.array(self.possible_actions).flatten()
    # def choose_action(self, obs, current_place, target_place, valid_path_matrix, epsilon = 1):
    #     epsilon = max(epsilon, self.epsilon) 
    ...
    #     return action
    def choose_action(self, obs, current_place, target_place, valid_path_matrix, epsilon):
        epsilon = max(epsilon, self.epsilon)
        if np.random.uniform() < epsilon:  # greedy
            # state
            state = torch.from_numpy(obs).float().unsqueeze(0).to(self.device)  # array to torch
    ...
    def store_transition(self, s, a, r, s_, is_done):
        ...
        # print("error:", error)
        self.memory.add(error, (np.array(s), a, r, np.array(s_), is_done)) # Them vao experience replay o day neeee
        # print(self.memory)
        """"update NN"""
    ...
    def update_network(self):
        # print("updatenetwork:", state_action_values)
