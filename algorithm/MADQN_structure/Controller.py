...
class MADQNAgentController:
    ...
    def __init__(self, rmfs_scene, map_xdim, map_ydim, max_task, control_mode=1, state_number=4, expert_guiding=False):  # 
        ...
        '''training parameters'''
        self.simulation_times = 100000
        self.max_value = max_task*2 - 1
        self.max_value_times = 0
        self.duration_times = 150
        """"hyper parameter"""
    ...
    def create_agent(self, map_xdim, map_ydim):
        ...
        if self.control_mode == "train_NN":
            ...
            print("load NN")
            # policy_net = torch.load(os.path.join(self.storage_path, "rb_p.pt"))
            # target_net = torch.load(os.path.join(self.storage_path, "rb_t.pt"))
            policy_net = torch.load("/home/ubuntu/Doan/results su dung dc/layout_5/rb_p.pt")
            target_net = torch.load("/home/ubuntu/Doan/results su dung dc/layout_5/rb_t.pt")
    ...
    def model_run(self):  # mainloop for training/running
        ...
        for i_episode in range(self.simulation_times):
            if self.control_mode == "use_NN":
                ...
                self.agent.lr = 3.399999999998868e-05
            ...
            print("reward:", self.reward_acc)
            if self.lr_start_decay:  
                self.agent.change_learning_rate(times=150)  
...
###############        
    ...
    def choose_action(self, all_info, this_veh, conflict):  # all_info=[layout, current_place, target_place]
        # print(conflict)
    ...
    def save_neural_network(self, auto=False):
        ...
        if auto:
            ...
            torch.save(self.agent.policy_network, os.path.join(self.storage_path, "rb_test_p.pt"))
            torch.save(self.agent.target_network, os.path.join(self.storage_path, "rb_test_t.pt"))
            # self.logs.append(self.lifelong_reward)
            ...
            torch.save(self.agent.policy_network, os.path.join(self.storage_path, "rb_p.pt"))
            torch.save(self.agent.target_network, os.path.join(self.storage_path, "rb_t.pt"))
