...
class Scene:
    def __init__(self, layout, explorer_group):
        ...
        self.cell_width = 20        #Chỉnh kích cỡ ô vuông
    ...
    def run_mode(self, running_type="manual", smart_controller=None):
        ...
        """---main loop---"""
        while True:
            pygame.time.delay(100)      #Delay moi buoc di cua AGV
            self.running_time += 1
            if self.render:
            ...
            # print(self.conflict_AGV)

            for index, explorer in enumerate(self.explorer_group):
                """---state check---"""
                ...
                if running_type == "intelligent":
                    ...
                    input_action = self.smart_controller.choose_action(all_info, explorer.explorer_name, explorer.conflict)  # get action
                    #2 down, 1 right, 0 up, 3 left        trai phai 0 len xuong 1
                    if explorer.working == 1:
                        input_action = 4
                        explorer.working = 2
                    elif explorer.working == 2:
                        input_action = 4
                        explorer.working = 3
                    elif explorer.working == 3:
                        input_action = 4
                        explorer.working = 5
                    elif explorer.working == 5:
                        input_action = 4
                        explorer.working = 6
                    elif explorer.working == 6:
                        input_action = 4
                        explorer.working = 0
                    else:
                        if input_action == 0:
                            if self.check([all_info[index + 1][1][0], all_info[index + 1][1][1] - 1]):
                                input_action = self.dir.action_str_value(explorer.find_path_astar(self.explorer_group))                            
                        elif input_action == 1:
                            if self.check([all_info[index + 1][1][0] + 1, all_info[index + 1][1][1]]):
                                input_action = self.dir.action_str_value(explorer.find_path_astar(self.explorer_group))
                        elif input_action == 2:
                            if self.check([all_info[index + 1][1][0], all_info[index + 1][1][1] + 1]):
                                input_action = self.dir.action_str_value(explorer.find_path_astar(self.explorer_group))
                ...
                if running_type == "intelligent":
                    ...
                    self.patch_agv_icon(explorer)
                    is_end = True if (self.running_time >= self.max_training_steps) else is_end
    ...
    def check(self, a):
        ...
        for explorer in self.explorer_group:
            ...
            if a == [explorer.current_place[0], explorer.current_place[1]]:
                return True
        return False
    ...
    def refresh_screen(self, explorer):
        if self.render:
            ...
            # self.create_sidebar()  # tao sidebar
            # update screen
            self.screen.blit(self.interface, (self.interface_start_x, self.interface_start_y))
            # self.screen.blit(self.sidebar, (self.sidebar_start_x, self.sidebar_start_y))
            # pygame.display.quit()  
    ...
    #             current_x, current_y = self.position_rectify(explore.current_place[0], explore.current_place[1], is_agv=True)
    #             target_x, target_y = self.position_rectify(explore.target_place[0], explore.target_place[1], is_agv=True)
                
    #             # Interpolate the x and y positions
    #             current_x += (target_x - current_x) * explore.move_progress
    #             current_y += (target_y - current_y) * explore.move_progress

    #             agv_position = (current_x, current_y)
    #             self.interface.blit(agv_image, agv_position)
    #     else:
    #         pass
    ...
    def create_interface(self):
        ...
        for y_dim in range(self.y_width):
            for x_dim in range(self.x_width):
                pygame.draw.rect(self.interface, self.color_box.WHITE_COLOR, (  #   Thay doi vien cua scence day!!!!!!!!!!!!!!!!!
                ...
                if (x_dim + 1, y_dim + 1) in self.layout.storage_station_list:
                    if self.layout.layout[y_dim][x_dim] == 1.8:
                        ...
                        # img = pygame.transform.scale(img, (self.cell_width * self.AGV_icon_scale, self.cell_width * self.AGV_icon_scale))
                        # self.interface.blit(img, self.position_rectify(x_dim + 1, y_dim + 1))
                    elif self.layout.layout[y_dim][x_dim] == 2.0:
                        self.draw_block(self.interface, self.color_box.WHITE_COLOR, x_dim, y_dim)
                    else:
                        self.draw_block(self.interface, self.color_box.PINK_COLOR, x_dim, y_dim)
                        # img = pygame.image.load("icons/pod_full.png")
                        # img = pygame.transform.scale(img, (self.cell_width * self.AGV_icon_scale, self.cell_width * self.AGV_icon_scale))
...
def process_explorer(dir,create_info,explorer_group,patch_agv_icon,running_time,task_finished,explorer, running_type, expert_info, expert_name, expert_action):
    """Process a single explorer"""
    if not explorer.has_created:
        return
    if task_finished:
        print("time:", running_time)
        return expert_info, expert_name, expert_action
    if explorer.all_assigned:
        patch_agv_icon(explorer)
        return
    print("Astar")
    # if running_type in ["A_star", "auto"]:
    print("helloworld")
    input_action = explorer.find_path_astar(explorer_group)  # through strategy
    expert_info.append(create_info())
    expert_name.append(explorer.explorer_name)
    expert_action.append(dir.action_str_value(input_action))
    print(input_action)
    print(expert_name)
    print(expert_action)